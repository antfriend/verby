...
VAR       
        ...
        long the_param_list_address
        
OBJ
        v       : "VocalTract"

PUB start(the_param_pointer)
         
    v.start(@aa, -1, -1, -1) 'start tract, no pin outputs
    v.set_attenuation(1)   'same as default
    
    the_param_list_address := the_param_pointer
    set_gp_to_pitch(1)
    bEEt_to_silence
    v.go(1)
    return v.sample_ptr
...
PUB go_test(the_key) | the_pitch_adjust
    ...
    f1 := byte[the_param_list_address][3] '* 100 / 1953 '19.53 Hz    1st resonator frequency: 40 -> 781 Hz
    f2 := byte[the_param_list_address][4] '* 100 / 1953  '19.53 Hz   2nd resonator frequency: 56 -> 1094 Hz
    f3 := byte[the_param_list_address][2] '* 100 / 1953  '19.53 Hz   3rd resonator frequency: 128 -> 2500 Hz
    f4 := byte[the_param_list_address][1] '* 100 / 1953  '19.53 Hz   4th resonator frequency: 179 -> 3496 Hz

    aa := byte[the_param_list_address][0]
     
     the_pitch_adjust := Convert_param_0_to_12(byte[the_param_list_address][5])
     the_pitch_adjust := the_pitch_adjust - 6
    set_gp_to_pitch(the_key + the_pitch_adjust)
    'always set gp before calling ga_wrapper
    ga_wrapper(Convert_param_1_to_85(byte[the_param_list_address][7]))
    
    vp := Convert_param_1_to_85(byte[the_param_list_address][8])  '1 'rnd(4, 48)  
    vr := Convert_param_1_to_85(byte[the_param_list_address][9]) '80 'rnd(4, 52)     

    na := byte[the_param_list_address][14]
    nf := byte[the_param_list_address][15] '* 100 / 1953 
    
    fa := byte[the_param_list_address][10]
    ff := byte[the_param_list_address][13] '* 100 / 3906 '0-255 works best? 39.06 Hz  2344 Hz ("Sh")
  
    v.go(byte[the_param_list_address][6]*2)
    repeat 7
      v.go(1)
...
{' ******************************
 ' *       key pressed          *
    ...
    vp := Convert_param_1_to_85(byte[the_param_list_address][3])  '1 'rnd(4, 48)  
    vr := Convert_param_1_to_85(byte[the_param_list_address][4]) '80 'rnd(4, 52) 

    Select_Allophone(byte[the_param_list_address][0], the_key)
    'always set gp before calling ga_wrapper
    ga_wrapper(Convert_param_1_to_85(byte[the_param_list_address][2]))
    v.go(byte[the_param_list_address][1]*2)
...
{' ******************************
 ' *        key held            *
  ...
  'set_gp_to_pitch(the_key)
    ...
    Select_Allophone(byte[the_param_list_address][5], the_key)
    'always set gp before calling ga_wrapper
    ga_wrapper(Convert_param_1_to_85(byte[the_param_list_address][7]))
    v.go(byte[the_param_list_address][6]*2)
...
{' ******************************
 ' *       key released         *
    ...
    vp := Convert_param_1_to_85(byte[the_param_list_address][14])  '1 'rnd(4, 48)  
    vr := Convert_param_1_to_85(byte[the_param_list_address][15]) '80 'rnd(4, 52) 
   
    Select_Allophone(byte[the_param_list_address][10], the_key)
    'always set gp before calling ga_wrapper
    ga_wrapper(Convert_param_1_to_85(byte[the_param_list_address][13]))
    v.go(byte[the_param_list_address][11]*2)'50
...
PUB go_null
  ...
  Select_Allophone(byte[the_param_list_address][11], 1)
    ga_wrapper(0)
    aa := 0
    fa := 0
    na := 0
...
PRI Select_Allophone(the_param_value, the_key)

    set_gp_to_pitch(the_key)

    the_param_value := Thirtyfifth_value_of_param(the_param_value)
    case the_param_value
       0 : bEEt_to_silence
       1 : bEEt_to_silence
       2 : bEEt_to_silence
       3 : sOAp
...
PRI go_blah(the_key) 
  ...
  v.go(byte[the_param_list_address][1])

  setformants(730, 1050, 2500, 3480)
  aa := 20
  ga_wrapper((byte[the_param_list_address][1]/3))
  na := 100       'added
  nf := 200      'added
  v.go(4*byte[the_param_list_address][1])
...
PRI Convert_param_1_to_85(the_value)
  return the_value /3

PRI Thirtyfifth_value_of_param(param_value) : the_decimal_value
  return param_value/7

PRI Convert_param_0_to_12 (the_value)
  return the_value /21

PRI Convert_param_0_to_10 (the_value)
  return the_value /25
